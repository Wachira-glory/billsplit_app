...
// import { getUndaAuthClient, supabase } from '@/app/lib/supabaseClient';
...
//       // Use the VIEW for fetching to bypass the table lock cache
...
//       // Use the exact structure that worked in CURL
...
//           status: 'active',
//           idata: { 
//             is_default: true, // New channels can start as default
//             owner_id: user.id,
//             display_uid: newUid 
//           } 
//         }]);
...
//       // OPTIMISTIC UI: Switch the checkmark immediately
//       setChannels(prev => prev.map(ch => ({
//         ...ch,
//         idata: { ...ch.idata, is_default: ch.id === channelId }
//       })));

//       // 1. Get all IDs to update
//       const otherChannelIds = channels.filter(c => c.id !== channelId).map(c => c.id);

//       // 2. Set the chosen one to true (PostgreSQL JSONB merge)
//       await authSupa
//         .from('channels')
//         .update({ idata: { ...channels.find(c => c.id === channelId)?.idata, is_default: true } })
//         .eq('id', channelId);

//       // 3. Set others to false
//       if (otherChannelIds.length > 0) {
//         for (const id of otherChannelIds) {
...
//             <input required placeholder="Personal Till" className="w-full p-4 bg-slate-50 rounded-2xl outline-none font-bold border-2 border-transparent focus:border-purple-200 transition-all text-black" value={newName} onChange={e => setNewName(e.target.value)} />
//           </div>
//           <div className="space-y-1">
//             <label className="text-[10px] font-black text-slate-400 uppercase ml-4">Till / Paybill Number</label>
//             <div className="relative">
//               <input required placeholder="123456" className="w-full p-4 bg-slate-50 rounded-2xl outline-none font-mono font-bold border-2 border-transparent focus:border-purple-200 transition-all text-black" value={newUid} onChange={e => setNewUid(e.target.value)} />
//               <Hash className="absolute right-5 top-1/2 -translate-y-1/2 text-slate-300" size={18} />
//             </div>
//           </div>
//           <button disabled={isCreating} className="w-full py-5 bg-slate-900 text-white font-black rounded-2xl hover:bg-purple-600 disabled:opacity-50 transition-all flex items-center justify-center gap-2">
...
//             <div key={channel.id} className={`p-6 rounded-[2.5rem] border-2 flex items-center justify-between transition-all ${channel.idata?.is_default ? 'bg-white border-purple-500 shadow-xl' : 'bg-white border-slate-50'}`}>
...
//                   <p className="text-xs font-mono font-bold text-purple-500">{channel.idata?.display_uid || "Verified"}</p>
//                 </div>
//               </div>
//               <button onClick={() => setDefaultChannel(channel.id)} className={`px-6 py-3 rounded-xl font-black text-xs uppercase ${channel.idata?.is_default ? 'bg-emerald-500 text-white' : 'bg-slate-100 text-slate-500'}`}>
//                 {channel.idata?.is_default ? 'Active' : 'Use'}
//               </button>
//             </div>
//           ))
//         )}
